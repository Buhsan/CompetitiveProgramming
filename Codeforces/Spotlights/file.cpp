...
void solve(int tt){
  int n, m;
  cin >> n >> m;
  vector<vint> mapa(n, vint(m));
  vector<pii> zeros;
  map<int, Cell> ones_hort;
  map<int, Cell> ones_vert;
  REP(i,0,n){
    REP(j,0,m){
      cin >> mapa[i][j];
      if(mapa[i][j]) {
        ones_hort[i].cnt++;
        ones_vert[j].cnt++;
        if (ones_hort[i].pos.first == -1){
          ones_hort[i].pos = {j, j};
        }
        else {
          ones_hort[i].pos.first = min(ones_hort[i].pos.first, j);
          ones_hort[i].pos.second = max(ones_hort[i].pos.second, j);
        }
        if (ones_vert[j].pos.first == -1){
          ones_vert[j].pos = {i, i};
        }
        else {
          ones_vert[j].pos.first = min(ones_vert[j].pos.first, i);
          ones_vert[j].pos.second = max(ones_vert[j].pos.second, i);
          // if (i >= 0 && i < ones_vert[j].pos.first) ones_vert[j].pos.first = i;
          // else if (i < n && i > ones_vert[j].pos.second) ones_vert[j].pos.second = i;
        }
      }
  ...
  REPLIST(num, zeros) {
    // PAIR(num);
    pii hort = ones_hort[num.first].pos;
    pii vert = ones_vert[num.second].pos;
    // PAIR(hort);
    // PAIR(vert);
    // P(ones_hort[num.first].cnt);
    // P(ones_vert[num.second].cnt);
    int res = 0;
    if(hort.first >= 0 && (hort.first < num.second) && (hort.second > num.second)) res += 2;
    else if (ones_hort[num.first].cnt != 0) res += 1;
    if(vert.first >= 0 && (vert.first < num.first) && (vert.second > num.first)) res += 2;
    else if (ones_vert[num.second].cnt != 0) res += 1;
    // P(res);
    cnt += res;
  }
  PRINT(cnt);
