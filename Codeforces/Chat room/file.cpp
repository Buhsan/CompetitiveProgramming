...
bool search (vector<char> &trg, int trg_id, int index, string &s) {
  if (index > s.size()-1) return false;
    if (trg_id == trg.size()-1 && s[index] == trg[trg_id]) return true;
  if (s[index] == trg[trg_id]) {
    return search(trg, trg_id+1, index+1, s);
  }
  else{
    return search(trg, trg_id, index+1, s);
  }
  // return false;
}
