...
#define PPAIR(a) for (auto j : a) { cout<<"{"<<j.first<<" "<<j.second<<"} "; } cout<<endl;
...
void solve(int tt){
  ...
  sort(freqV.begin(), freqV.end(), [](pll x, pll y) {return x.second <y.second;});
  cout << freqV[0].first << " " << freqV[1].first - 1 << endl;
}
