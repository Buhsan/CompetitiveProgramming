...
void dfs(int v, vector<vint> &adj, vint &out, vector<bool> &visited) {
  visited[v] = true;
  REPLIST(u,adj[v]) {
    if (!visited[u]) dfs(u, adj, out, visited);
  }
  out.push_back(v);
}


void scc(vector<vint> &adj, vector<vint> &components, vector<bool> visited, vector<vint> &adjT) {
  int n = adj.size();
  vint order;
  visited.assign(n, false);

  REP(i,0,n) {
    if (!visited[i]) dfs(i,adj,order, visited);
  }

  // vector<vint> adj_rev(n);
  // REP(i,0,n){
  //   REPLIST()
  // }
  visited.assign(n, false);
  ...
  REPLIST(v, order) {
    if(!visited[v]) {
      vint component;
      dfs(v, adjT, component, visited);
      components.push_back(component);
      int index = MIN(component);
      if (index == component.size()) PRINT(5);
      int root = component[index];
      REPLIST(u,component) {
        roots[u] = root;
      }
    }
