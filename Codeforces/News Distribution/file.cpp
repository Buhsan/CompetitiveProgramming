...
void make_set(vector<int> &nodoPadre, vint &size, int v) {
  nodoPadre[v] = v;
  size[v] = 1;
}
...
int find_set(vector<int> &nodoPadre, int v) {
    if (v == nodoPadre[v])
        return v;
    return nodoPadre[v] = find_set(nodoPadre, nodoPadre[v]);
}

void union_sets(vector<int> &nodoPadre, vint &size, int a, int b) {
...
void solve(int tt){
  ...
  REP(i,0,m){
    ...
    REP(j,0,k){
      ...
      if(find_set(dsu, num) != find_set(dsu, first)) union_sets(dsu,size,num,first);
    }
  }
  REP(i,0,n) {
    cout << size[find_set(dsu,i)] << " ";
    // PRINT(size[find_set(dsu,i)]);
  }
  cout<<endl;
}
