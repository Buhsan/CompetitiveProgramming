...
void solve(int tt){
    ...
    if(n-1 < 5){
        ...
        REP(i,0,s.size()){
            string aux = "";
            if(i<m) aux.insert(aux.size() > 0 ? aux.size()-1 : 0, full+1, s[i]);
            else if (i==m) aux.insert(aux.size() > 0 ? aux.size()-1 : 0, full+1, s[i]);
            else if (i > m ) aux.insert(aux.size() > 0 ? aux.size()-1 : 0, full, s[i]);
            res+=aux;
        }
    }
    PRINT(res);
