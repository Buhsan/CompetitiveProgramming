...
struct Player {
  ...
  vector<vint> game = {{-1, 0, 2},{0, -1, 1},{2, 1, -1}};
  vector<vint> game_inv = {{-1, 1, 0},{1, -1, 2},{0, 2, -1}};
  vector<string> elem = {"piedra", "tijera", "papel"};
  map<string, int> rules = {{"piedra", 0}, {"tijera", 1}, {"papel",2}};
...
struct WinOP {
  ...
  // rules[1] = ""
  T operator()(T a, T b) const {
    // cout << a.weapon << "(" << a.layer << ") " << "<--> " << b.weapon << "(" << b.layer << ")" << endl;
    int aa = rules[a.weapon];
    vector<T> res(3);
    int bb = rules[b.weapon];
    int id_win;
    if (a.layer % 2) id_win  = game_inv[aa][bb];
    else id_win = game[aa][bb];
    res[aa] = a;
    res[bb] = b;
    if (id_win == -1) { return {a.weapon, a.layer+1};}
    return {elem[id_win], res[id_win].layer+1};
  } T id() const {
    // return LLONG_MAX;
    return {"", 0};
  }
...
//     int aa = rules[a];
//     int bb = rules[b];
//     int id_win = game[aa][bb];
//     if (id_win == -1) return a;
//     return elem[id_win];
//   } T id() const {
//     // return LLONG_MAX;
//     return "";
...
void solve(int tt){
  ...
  vector<Player> pl(a.size());
  FILL(a);
  REP(i,0,a.size()) {
    pl[i] = {a[i], 0};
  }
  // PLIST(a);
  SegTree<Player, WinOP> segTree(pl);
  REP(i,0,m) {
    int aux;
    string s;
    // P(i);
    cin >> aux >> s;
    segTree.update(aux-1,{s, 0} );
    // PRINT("RESULT: ");
    Player res = segTree.query(0, a.size()-1);
    // P(1);
    PRINT(res.weapon);
  }
