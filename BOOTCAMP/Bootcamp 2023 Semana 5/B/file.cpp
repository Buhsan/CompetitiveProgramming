...
set<pii> directions = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}};
 
void dfs(vector<string> &lab, pii pos, int length, set<pii> &visited) {
  int i = pos.first;
  int j = pos.second;
  if (lab[i][j] == 'S') {
    PRINT(length);
  ...
  REPLIST(dir,directions) {
    int in = i + dir.first;
    int jn = j + dir.second;
    if (in>= 0 && in < lab.size() && jn>=0 && jn < lab[0].size() && lab[in][jn] != '#' && !visited.count({in,jn}) ) {
      visited.insert({in,jn});
      dfs(lab, {in,jn}, length+1, visited);
    }
  }
}
