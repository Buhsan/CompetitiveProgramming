...
void solve(int tt){
  ...
  FILL(candies);
  FILL(oranges);
  int min_candies = MIN(candies);
  int min_oranges = MIN(oranges);
  ll res = 0;
  REP(i,0,n){
    if (i != min_candies && i != min_oranges){
      int m = min(candies[i]-candies[min_candies], oranges[i]-oranges[min_oranges]);
      int mm = max(candies[i]-candies[min_candies], oranges[i]-oranges[min_oranges]);
      res+=(m+(mm-m));
    }
    else{
      res+=(candies[i]-candies[min_candies]);
      res+=(oranges[i]-oranges[min_oranges]);
    }
  }
  PRINT(res);
}
