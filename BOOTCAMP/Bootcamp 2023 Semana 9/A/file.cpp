...
int trip(vector<vint> &dp, vector<vint>& v, int i, int j) {
  if (j < 0 || i < 0) return INT_MIN;
  if (dp[i][j] != INT_MIN) return dp[i][j];
  
  return dp[i][j] = v[i][j] + max(trip(dp, v, i-1, j), trip(dp, v, i, j-1));
}
...
void solve(int tt){
  ...
  vector<vint> dp (n, vint(m, INT_MIN));
  dp[0][0] = mapa[0][0];
  PRINT(trip(dp, mapa, n-1, m-1));
  // REP(i,0,n) {
  //   PRINTLIST(dp[i]);
  // }
