...
vll prefix_sum(vll a){
  ...
  vll pref_sum(a.size());
  if (dp[a[0]] == -1) {
    bool res = isPrime(a[0]);
    dp[a[0]] = res;
    pref_sum[0] = res ? a[0] : 0;
  }
  else {
    pref_sum[0] = dp[a[0]] == 1 ? a[0] : 0;
  }
  REP(i, 1, a.size()){
    if (dp[a[i]] == -1) {
      bool res = isPrime(a[i]);
      dp[a[i]] = res;
      pref_sum[i] = pref_sum[i - 1] + (res ? a[i] : 0);
    }
    else {
      pref_sum[i] = pref_sum[i - 1] + (dp[a[i]] == 1 ? a[i] : 0);
    }
    // pref_sum[i] = pref_sum[i - 1] + (isPrime(a[i]) ? a[i] : 0);
  }
  return pref_sum;
