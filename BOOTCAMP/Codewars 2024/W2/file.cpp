...
struct SumOp {
  ...
  T operator()(T a, T b) const {
    return (isPrime(a) ? a : 0) + (isPrime(b) ? b : 0);
  }
...
struct SumPrimeOP {
  ...
  T operator()(T a, T b) const {
    ...
    if (a.isLeaf && isPrime(a.val)) newNum += a.val;
    if (!a.isLeaf) newNum += a.val;
    if (b.isLeaf && isPrime(b.val)) newNum += b.val;
    if (!b.isLeaf) newNum += b.val;
...
void solve(int tt){
  ll n;
  cin >> n;
  vector<NodePrime> a(n);
  REP(i,0,n){
    cin >> a[i].val;
    a[i].isLeaf = true;
  }
  ll r; cin >> r;
  SegTree<NodePrime, SumPrimeOP> segTree(a);
  REP(i,0,r) {
    char oper;
    cin >> oper;
    if (oper == 'Q'){
