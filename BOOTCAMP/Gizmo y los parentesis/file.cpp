...
struct BracketOp {
  ...
  T operator()(T a, T b) const {
    ...
    return {a.open+b.open-m, a.close+b.close-m, m + a.matched + b.matched};
  }
...
void solve(int tt){
  ...
  REP(i,0,s.size()) {
    ...
    else br.push_back({0,1,0});
  }
  int n;
  cin >> n;
  SegTree<Bracket, BracketOp> segTree(br);
  REP(i,0,n) {
    int a,b;
    cin >> a >> b;
    Bracket bra = segTree.query(a-1, b-1);
    PRINT(bra.matched*2);
  }
}
