...
void solve(int tt){
  int n, m;
  cin >> n >>m;
  vector<queue<string>> st(n);
  string s;
  int cnt = 0;
  while(getline(cin, s) && m>0) {
    if (s != "") {
      n--;
      string action(s.begin(), s.begin()+s.find(" "));
      if (action == "a") {
        string fila(s.begin()+s.find(" ")+1, s.begin()+s.rfind(" "));
        string name(s.begin()+s.rfind(" "), s.end());
        int f = stoi(fila);
        st[f-1].push(name);
        cnt++;
      }
      else if (action == "o") {
        string id_pers (s.begin()+s.find(" ")+1, s.end());
        int id = stoi(id_pers) - 1;
        if (st[id].empty()) {
          cout << "- No hay nadie en la fila " << id+1 <<endl;
        }
        else {
          string n = st[id].front(); st[id].pop();
          cout << "- Atendiendo a " << n << endl;
          cnt--;
        }
      }
      else if (action == "p") {
        string id_pers (s.begin()+s.find(" ")+1, s.end());
        int id = stoi(id_pers) - 1;
        cout << "- Personas en la fila " << id+1 << ": " << st[id].size() << endl;
      }
    }
  }
  cout << "Personas por atender: " << cnt << endl;
}
