...
bool dfs (set<pii> &dirs, pii queen) {
  stack<pii> st;
  st.push({0,0});
  while (!st.empty()) {
    pii v = st.top();
    st.pop();
    if (v == queen) return true;
    pii nv = v;
    REPLIST(d, dirs) {
      nv = {nv.first+d.first, nv.second+d.second};
    }
    st.push(nv);
  }
  return false;
...
void solve(int tt){
  ...
  REP(i,0,n) {
    char c;
    cin >> c;
    if (c == 'N') dirs.insert({0,1});
    else if (c == 'E') dirs.insert({1,0});
    else if (c == 'S') dirs.insert({0,-1});
    else if (c == 'W') dirs.insert({-1, 0});
  }
  yn(dfs(dirs, {a,b}));
}
