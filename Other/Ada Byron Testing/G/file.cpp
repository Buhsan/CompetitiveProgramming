...
void solve(int tt){
  ...
  for (int tt = 1; tt <= t; tt++) {
    ...
    queue<State> q;
    State inicial = {start, vector<bool>(n, false)};
    inicial.visited[start] = true;
    q.push(inicial);
    int ans = 0;

    while (!q.empty()) {
      State actual = q.front();
      q.pop();

      if (actual.actual == end) {
        ans++;
        continue;
      }

      for (int vecino : adj[actual.actual]) {
        if (!actual.visited[vecino]) {
          State newState = actual;
          newState.actual = vecino;
          newState.visited[vecino] = true;
          q.push(newState);
        }
      }
    ...
    if (!ans) {
      cout << "No paths connecting nodes " << start + 1 << " and " << end + 1 << endl;
    } else if (ans == 1) {
      cout << "Found one path connecting node " << start + 1 << " with node " << end + 1 << endl;
    } else {
      cout << "Found " << ans << " paths connecting node " << start + 1 << " with node " << end + 1 << endl;
    }
  }
}
