...
void solve(int tt){
  int n, capacidadMax;
  cin >> n >> capacidadMax;
  vector<Medicamento> meds(n);

  int precioMin = 0;
  REP (i, 0, n) {
    cin >> meds[i].vol >> meds[i].price >> meds[i].boxes;

    precioMin += meds[i].price * meds[i].boxes;
    capacidadMax -= meds[i].vol * meds[i].boxes;
  }

  REP (i, 0, n) {
    int maxN = capacidadMax / meds[i].vol;
    REP (j, 0, maxN) meds.push_back(meds[i]);
  }
  
  vector<vint> dp (meds.size()+1, vint(capacidadMax + 1));

  int ans = 0;
  REP(i,1,meds.size()+1){
    REP(j,0,capacidadMax+1){
      dp[i][j] = max(
        dp[i-1][j],
       (j - meds[i-1].vol >= 0 ? dp[i-1][j-meds[i-1].vol] + meds[i-1].price: INT_MIN)
      );
    }
  }
  
  cout << dp[meds.size()][capacidadMax]+ precioMin << endl;
}
