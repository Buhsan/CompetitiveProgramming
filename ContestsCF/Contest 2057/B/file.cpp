...
void solve(int tt){
    ...
    // if(tt==945){
    //     cout<<k<<endl;
    //     // for (int i = 0; i < a.size(); ++i) { cout << a[i] << 0; } cout<<endl;
    // }
    // vector<pii> freqv;
    // REPLIST(i,freq){
    //     freqv.push_back(i);
    // }
    // sort(freqv.begin(),freqv.end(), [&](pii &x, pii &y) { return x.second < y.second; });
    // int rem = 0;
    // if(k>0){
    //     REP(i,0,n){
    //         if(k >= freqv[i].second){
    ...
    //     }
    // }
    // int ab = freqv.size()-rem;
    // int res = max(1, ab);
    // PRINT(res);
    sort(a.begin(), a.end(), [&](int x, int y) { return freq[x] == freq[y] ? x < y : freq[x] < freq[y];});
    set<int> b (a.begin()+k, a.end());    
    int res = b.size() == 0 ? 1 : b.size();
    PRINT(res);
}
