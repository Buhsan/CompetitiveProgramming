...
bool simulation(int n, int m, vector<pair<int,vint>> ans) {
  int prev = -1;
  REP(j,0,m) {
    REP(i,0,n) {
      if (prev < ans[i].second[j]) {prev = ans[i].second[j];}
      else return false;
    }
  }
  return true;
...
bool check_cards(vector<vint> v, vector<pair<int,vint>> ans) {
  int n = v[0].size();
  int m = v.size();
  REP(j,0,m) {
    vint aux;
    int num;
    REP(i,0,n){
      aux.push_back(ans[i].second[j]);
    }
    // PLIST(aux);
    // PLIST(v[j]);
    if (aux != v[j]) return false;
  }
  return true;
}
...
void solve(int tt){
  int n, m;
  cin >> n >>m;
  vector<vint> v(m, vint(n, 0));
  vector<pair<int,vint>> ans;
  REP(i,0,n) {
    vint a(m);
    FILL(a);
    SORTINCR(a);
  ...
  sort(ans.begin(), ans.end(), [](pair<int,vint> x, pair<int,vint> y) { return x.second[0] < y.second[0]; });
  REP(i,0,m) {
    SORTINCR(v[i]);
  }

  if (!simulation(n,m,ans)) PRINT(-1);
  else {
    REPLIST(i,ans) {
      cout << i.first << " ";
    }
