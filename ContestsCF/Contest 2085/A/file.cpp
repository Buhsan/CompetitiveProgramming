...
void solve(int tt){
  ...
  // P(k);
  while(high >= low) {
    // cout << s[low] << " vs " << s[high] << " " << (s[low] > s[high]) << endl;
    if (s[low] != s[high] && s[low] < s[high]) {res = true; break;}
    if (s[low] != s[high] && s[low] > s[high] && k > 0) k--, enter = true; 
    else if (s[low] != s[high] && s[low] > s[high] && k<=0) {res = false; break;}
    low++;
    high--;
  }
  if (!enter && k > 0) res = true;
