...
void solve(int tt){
  ...
  vint a(n);
  FILL(a);
  vint sub1(a.begin(), a.begin()+l);
  vint range (a.begin()+l, a.begin()+r+1);
  vint sub2(a.begin()+r+1, a.end());
  PLIST(a);
  PLIST(sub1);
  PLIST(range);
  PLIST(sub2);
  int mr = range[MAX(range)];
  int m1 = INT_MAX;
  int m2 = INT_MAX;
  if (!sub1.empty()) m1 = sub1[MIN(sub1)];
  if (!sub2.empty()) m2 = sub2[MIN(sub1)];
  REP(i,0,range.size()) {

  }
  int res_min = min(m1, m2);
  int sum = accumulate(range.begin(), range.end(), 0);
  P(sum);
  PRINT(sum-mr+res_min);
