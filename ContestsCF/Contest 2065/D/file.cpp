...
void solve(int tt){
  ...
  REP(i,0,n){
    FILL(a[i]);
    sum[a[i]] = accumulate(a[i].begin(), a[i].end(), 0);
  }
  sort(a.begin(), a.end(), [&](vint x, vint y) { return sum[x] > sum[y]; });
  ll res = 0;
  // vint b (a[0].begin(), a[0].end());
  vint b;
  vint pref(n*m);
  // pref[0] = a[0][0];
  ...
  REP(i,0,n){
    REP(j,0,m){
      int id = (i*m+j);
      if (id-1 < 0) pref[id] = a[i][j], res+=a[i][j];
      else {
        pref[id]= pref[id-1] + a[i][j];
        res += pref[id];
      }
    }
  }
  ...
  // pref[0]=b[0];
  // REP(i,1,n*m+1){
  //   res = accumulate(b.begin(), b.begin()+i, res);
  // }
  PRINT(res);
}
