...
void solve(int tt){
  int n, x; cin >> n >> x;
  vint prices(n); FILL(prices);
  vint pages(n); FILL (pages);
  vector<vint> dp(n + 1, vint(x+1));
  int ans = 0;
  // dp[i][j] es el máximo de páginas que puedes conseguir hasta el índice i con j euros
  REP(i,1,n+1) {
    REP(j,0,x+1) {
      dp[i][j] = max(
        dp[i-1][j],
        (j - prices[i - 1] >= 0 ? dp[i-1][j-prices[i-1]] + pages[i-1] : INT_MIN)
      );
      ans = max(ans, dp[i][j]);
    }
  }
