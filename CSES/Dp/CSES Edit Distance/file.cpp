...
int levDist(int i, int j, string &a, string &b, vector<vint> &dp) {
  if (i == 0 || j == 0) return dp[i][j] = max(i, j);
  if (dp[i][j] != -1) return dp[i][j];
  return dp[i][j] = min({
    max(i, j),
    levDist(i - 1, j - 1, a, b, dp) + (a[i - 1] != b[j - 1]),
    levDist(i, j - 1, a, b, dp) + 1,
    levDist(i - 1, j, a, b, dp) + 1
  });
}
...
void solve(int tt){
  string s, s1;
  cin >> s >> s1;
  vector<vint> dp(s.size() + 1, vint(s1.size() + 1, -1));
  PRINT(levDist(s.size(), s1.size(), s, s1, dp));
  // REP (i, 0, dp.size()) {
  //   REP (j, 0, dp[i].size()) {
  //     cout << dp[i][j] << " ";
  //   }
