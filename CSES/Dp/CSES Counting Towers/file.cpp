...
// ll countWays(ll n, ll j, vector<vector<ll>> &dp) {
//   if (dp[n][j] != -1) return dp[n][j];
 
//   if (j == 0) {
//     return dp[n][j] = (2 * countWays(n - 1, 0, dp)%MOD + countWays(n - 1, 1, dp)%MOD) % MOD;
//   }
//   return dp[n][j] = (countWays(n - 1, 0, dp)%MOD + 4 * countWays(n - 1, 1, dp)%MOD) % MOD;
// }
...
ll countWays(int n) {
  ...
  REP(i,1,n){
    REP(j,0,2){
      if(j==0) dp[i][j] = (2*dp[i-1][0] + dp[i-1][1]) % MOD;
      else if (j == 1) dp[i][j] = (dp[i-1][0] + 4*dp[i-1][1]) % MOD;
    }
  }
