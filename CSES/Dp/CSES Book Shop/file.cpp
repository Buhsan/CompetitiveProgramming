...
// int maxPages(int i, int j, vector<int> &precio, vector<int> &paginas, vector<vector<int>> &dp) {
//   if (i < 0) return 0;
//   if (dp[i][j] != -1) return dp[i][j];

//   dp[i][j] = max(
//     maxPages(i - 1, j, precio, paginas, dp), // no cogemos
//     (j - precio[i] >= 0 ? maxPages(i - 1, j - precio[i], precio, paginas, dp) + paginas[i] : INT_MIN) // cogemos
//   );
...
//   cout << maxPages(n, x, precio, paginas, dp) << endl;
// }
...
void solve(int tt) {
  ...
  for (int i = 1; i <= n; i++) {
    for (int j = 0; j <= x; j++) {
      dp[i][j] = dp[i - 1][j]; // no coger
      if (j + precio[i - 1] <= x) {
        dp[i][j] = max(dp[i][j], dp[i - 1][j + precio[i - 1]] + paginas[i - 1]);
      }
      ans = max(dp[i][j], ans);
    }
  }
