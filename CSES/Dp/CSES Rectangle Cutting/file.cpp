...
// minimo numero de movimientos = dp[i][j]
int minMoves(int i, int j, vector<vector<int>> &dp) {
  ...
  REP (x, 1, i) {
    ...
    dp[i][j] = min(dp[i][j], 1 + minMoves(primerCorte, j, dp) + minMoves(segundoCorte, j, dp));
  }
  REP (x, 1, j) {
    ll primerCorte = x;
    ll segundoCorte = j - x;

    dp[i][j] = min(dp[i][j], 1 + minMoves(i, primerCorte, dp) + minMoves(i, segundoCorte, dp));
  }
