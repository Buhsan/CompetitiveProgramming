...
void solve(int tt){
  ll n, m, k; cin >> n >> m >> k;
  vll aplicants(n); FILL(aplicants);
  vll apartments(m); FILL(apartments);
  SORTINCR(aplicants);
  SORTINCR(apartments);
  ll apartment = 0;
  ll res = 0;
  while (apartment < m) {
    REP(i,0,n) {
      if (aplicants[i] - k <= apartments[apartment] && apartments[apartment] <= aplicants[i] + k) {
        apartment++;
        res++;
      }
      else if (aplicants[i] - k > apartments[apartment] || apartments[apartment] > aplicants[i] + k) apartment++;
    }
  }
  PRINT(res);
}
