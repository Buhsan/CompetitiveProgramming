...
void dfs(int start, vector<vint>& adj, vector<bool> &visited) {
  stack<int> st;
  st.push(start);
  visited[start] = true;
  while(!st.empty()) {
    ...
    REPLIST(i,adj[v]) {
      if(!visited[i]) st.push(i), visited[i] = true;
    }
  }
}
...
void solve(int tt){
  ...
  vector<int> cc;
  REP(i,0,n){
    if(!visited[i]) dfs(i, adj, visited), cnt++, cc.push_back(i); 
  }
  PRINT(cnt - 1);
  REP (i, 0, cnt - 1) {
    cout << cc[i % cc.size()] + 1 << " " << cc[(i + 1) % cc.size()] + 1 << endl;
  }
}
