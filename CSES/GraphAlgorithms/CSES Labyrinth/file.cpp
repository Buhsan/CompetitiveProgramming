...
void solve(int tt){
  ...
  REP(i,0, n) {
    REP (j, 0, m) {
      ...
      if (mapa[i][j] == 'A') start = {i, j};
      else if (mapa[i][j] == 'B') end = {i, j};
    }
  }
  ...
  while (!q.empty()) {
    ...
    for (int dir = 0; dir < directions.size(); dir++) {
      int newI = actual.first + directions[dir].first;
      int newJ = actual.second + directions[dir].second;

      if (newI >= 0 && newI < mapa.size() && newJ >= 0 && newJ < mapa[newI].size() && mapa[newI][newJ] != '#' && prev[newI][newJ] == -1) {
        prev[newI][newJ] = dir;
        q.push({newI,newJ});
  ...
  if (prev[end.first][end.second] == -1) {
    PRINT("NO");
    return;
  }
  else {
