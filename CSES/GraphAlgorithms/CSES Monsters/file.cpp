...
void solve(int tt){
  ...
  vector<string> mapa;
  pii start;
  vector<vint> dist(n, vint(m, INT_MAX));
  queue<State> monsters, q;
  REP(i,0,n) {
    string s; cin >> s;
    mapa.push_back(s);
    REP(j,0,m) {
      if (s[j] == 'M') monsters.push({{i,j},0}), dist[i][j] = 0;
      else if (s[j] == 'A') start = {i,j};
    }
  }

  while(!monsters.empty()) {
    State monst = monsters.front();
    int i = monst.pos.first;
    int j = monst.pos.second;
    monsters.pop();
    // PAIR(monst.pos);
    dist[i][j] = min(dist[i][j], monst.len);

    REPLIST(dir, directions) {
      int newI = i + dir.first;
      int newJ = j + dir.second;
      // cout << newI << " " << newJ << endl;
      if (newI >= 0 && newI < n && newJ >= 0 && newJ < m && mapa[newI][newJ] != '#' && dist[newI][newJ] == INT_MAX) {
        monsters.push({{newI, newJ}, monst.len+1});
      }
    }
  }
  vector<vint> prev (n, vint(m, -1));
  prev[start.first][start.second] = -2;
  q.push({start,0});
  bool exit = false;
  pii current;
  while(!q.empty()) {
    ...
    REP(dir, 0, directions.size()) {
      int newI = i + directions[dir].first;
      int newJ = j + directions[dir].second;

      if (newI >= 0 && newI < n && newJ >= 0 && newJ < m && mapa[newI][newJ] != '#' && prev[newI][newJ] == -1 && len+1 < dist[newI][newJ]) {
        // P(len);
        // P(dist[newI][newJ]);
        prev[newI][newJ] = dir;
        q.push({{newI,newJ}, len+1});
      }
    }
  }
  if(!exit) {PRINT("NO"); return;}
  PRINT("YES");
  string res;
  while (current != start) {
    int dir = prev[current.first][current.second];
    res += directionChars[dir];
    current.first -= directions[dir].first;
    current.second -= directions[dir].second;
  }
