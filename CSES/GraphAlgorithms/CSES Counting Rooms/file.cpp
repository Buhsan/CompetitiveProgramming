...
void dfs(vector<vector<bool>> &visited, vector<string> &map, pii start){
  stack<pii> st;
  st.push(start);
  while(!st.empty()) {
    pii v = st.top();
    ...
    REPLIST(dir, directions) {
      int new_i = v.first + dir.first;
      int new_j = v.second + dir.second;
      if (new_i >= 0 && new_i < map.size() && new_j >= 0 && new_j < map[0].size() && map[new_i][new_j] != '#' && !visited[new_i][new_j]) {
        st.push({new_i, new_j});
