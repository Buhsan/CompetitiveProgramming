  ...
  REP (i, 0, n) {
    if (!visited[i] && !solved) {
      ...
      while(!q.empty()) {
        ...
        REPLIST(vecino, adj[v]){
          // cout <<"vecino: " << vecino + 1 << " v: " << v + 1<< " len: " << len << endl;
          if (vecino == v) continue;
     
          if(!visited[vecino]){
            prev[vecino] = v;
            q.push({vecino, len+1, v});
          }
          else if (len > 2  && prev[vecino] != prev[v] && vecino != prev[v]){
            solved = true;
            // P(2);
            prev[vecino] = v;
            // cout << prev[vecino] +1 << " "<< prev[v] + 1 << endl;
            int current = vecino;
            vint path;
            do {
              // cout << "current: " << current + 1 << " prev: " << prev[current]+1 << " v: " << v + 1<< endl;
              path.push_back(current+1);
              // PLIST(path);
              current = prev[current];
            } while (current != vecino && current != -1);
