...
// Precompute All-Pairs Shortest Paths for O(1) routing speed
...
int main() {
  // Ultra-fast I/O for competitive programming
  ...
  while (delivered_count < N) {
    ...
    for (int i = 1; i <= T; ++i) {
      ...
      while (!station_waiting[tr.pos].empty() && tr.capacity_left > 0) {
        ...
        city_demand[tr.pos]--; // Remove from global demand
    ...
    for (int i = 1; i <= T; ++i) {
      ...
      if (tr.passengers.empty() && tr.target_city != -1) {
        if (city_demand[tr.target_city] <= incoming_cap[tr.target_city]) {
          tr.target_city = -1; // Demand met, cancel commitment
        } else {
    ...
    for (int i = 1; i <= T; ++i) {
      ...
      if (!tr.passengers.empty()) {
        // Have passengers: Elevator routing to closest destination
        ...
        // Empty train: Find closest city with unmet demand
        int best_v = -1, best_dist = 1e9;
        for (int v = 1; v <= V; ++v) {
          if (city_demand[v] > incoming_cap[v]) {
            int d = dist_matrix[tr.pos][v];
            if (d < best_dist) {
              best_dist = d;
              best_v = v;
            }
    ...
    // Anti-infinite loop failsafe
    if (tick > 100000)
      break;
  }

  // BD output generation
  cout << timeline.size() << "\n";
