...
// int palindrom (vector<vint> dp, string &s, int i, int j) {
//   int len = j-i+1;
//   if (len < 0) return 0;
//   if (len == 0) return dp[i][j] = 0;
//   if (len == 1) return dp[i][j] = 1;
//   if (dp[i][j] != -1) return dp[i][j];

//   if (len == 2 && s[i-1] == s[j-1]) return 2;
//   if (len > 2 && s[i-1] == s[j-1] && palindrom(dp, s, i + 1, j - 1)) return len;
...
bool isPalindrome(string &s, vector<vector<int>> &dp, int i, int j) {
  if (i < 0 || j < 0) return 0;
  if (i > j) return 0;
  if (dp[i][j] != -1) return dp[i][j];
  ...
  int len = j - i + 1;
  if (len == 1) answer = 1;
  else if (len == 2 && s[i - 1] == s[j - 1]) answer = 1;
  else if (len > 2 && s[i - 1] == s[j - 1] && isPalindrome(s, dp, i + 1, j - 1)) answer = 1;
...
int longestPalindrome(string &s) {
  ...
  for (int i = 1; i < s.size() + 1; i++) {
    for (int j = i; j < s.size() + 1; j++) {
      if (isPalindrome(s, dp, i, j) && answer < j - i + 1) {
        answer = j - i + 1;
      }
    }
  }
...
void solve(int tt){
  string s;
  while(cin >> s) {
    vector<vint> dp (s.size(), vint(s.size()));
    ...
    //     if (len == 0) dp[i][j] = 0;
    //     else if (len == 1) dp[i][j] = 1;
    //     else if (len == 2 && s[i] == s[j]) dp[i][j] = 2;
    //     else if (len > 2 && s[i] == s[j] && dp[i + 1][j - 1]) dp[i][j] = len;
    //     ans = max(ans, dp[i][j]);
    //   }
    // }
    // cout << ans << endl;
