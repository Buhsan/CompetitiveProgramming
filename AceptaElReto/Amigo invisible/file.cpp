...
bool yn(bool cond, string tr = "SI", string fls = "NO") {
    if (cond) PRINT(tr);
    else PRINT(fls);
    return cond;
}
...
void solve(int tt){
  ...
  while(cin >> p >> a) {
    if (!p && !a) break;
    vector<bool> recibir(p);
    vector<bool> regalar(p);
    int rec_zero = p-a;
    REP(i,0,a) {
      int q, w; cin >> q >> w; q--; w--;
      regalar[q] = true;
    ...
    bool res = false;
    if (rec_zero == 0) res = true;
    else if (rec_zero < 0) res = true;
    else if (rec_zero > 2) res = false;
    else if (rec_zero == 1) res = true;
    else if (rec_zero == 2) {
      REP(i,0,p){
        if (regalar[i] == 0 && recibir[i] == 0) {res = true; break;}
      }
    }
    yn(res);
  }
