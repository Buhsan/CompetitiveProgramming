...
void solve(int tt){
  ...
  while (cin >> n) {
    ...
    REP (i, 0, m) {
      ...
      origen--, destino--;
      adj[origen].push_back({destino, tiempo});
      // adj[destino].push_back({origen, tiempo});
    }

    priority_queue<State, vector<State>, function<bool(State, State)>> pq([](const State& a, const State &b) {
      return a.tiempo > b.tiempo;
    });
    vector<int> bestTiempo(n, INT_MAX);

    pq.push({0, tiemposCarga[0]});
    bestTiempo[0] = tiemposCarga[0];
    while (!pq.empty()) {
      State current = pq.top();
      pq.pop();

      int pagina = current.pos;
      int tiempo = current.tiempo;

      if (pagina == n - 1) {
        break;
      }

      for (pair<int, int> par : adj[pagina]) {
        int vecino = par.first;
        int tiempoAdicional = par.second;

        int newTiempo = tiempo + tiempoAdicional + tiemposCarga[vecino];

        if (bestTiempo[vecino] > newTiempo) {
          bestTiempo[vecino] = newTiempo;
          pq.push({vecino, newTiempo});
        }
      }
    }

    cout << (bestTiempo[n - 1] == INT_MAX ? "IMPOSIBLE" : to_string(bestTiempo[n - 1])) << endl;
  }
}
