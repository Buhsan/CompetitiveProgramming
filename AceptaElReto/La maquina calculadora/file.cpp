...
void solve(int tt){
  ...
  while (cin >> a >> b) {
    ...
    while(!q.empty()) {
      ...
      int plus = (v+1)%10000;
      if (moves[v] + 1 < moves[plus]) {
        moves[plus] = moves[v] + 1;
        q.push(plus);
      }
      int product = (v*2)%10000;
      if (moves[v] + 1 < moves[product]) {
        moves[product] = moves[v] + 1;
        q.push(product);
      }
      int division = (v/3)%10000;
      if (moves[v] + 1 < moves[division]) {
        moves[division] = moves[v] + 1;
        q.push(division);
      }
    }
  }
