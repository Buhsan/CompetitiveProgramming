...
string formatNumber(int n, int interrogantes) {
  ostringstream oss;
  oss << setw(interrogantes) << setfill('0') << n;
  return oss.str();
}
...
void solve(int tt){
  ...
  s = s.substr(0, 8);
  string inferior = s, superior = s;
  REP (i, 0, s.size()) {
    if (s[i] == '?') inferior[i] = '0', superior[i] = '9';
  }
  int numInf = stoi(inferior);
  int numSup = stoi(superior);

  cout << ceil(numSup / 23.0f) - ceil(numInf / 23.0f) << endl;
}
