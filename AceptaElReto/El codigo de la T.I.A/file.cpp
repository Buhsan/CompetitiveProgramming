...
int countWays(int n, string &msg, vector<int> &dp, set<string> &words) {
  if (n < 0) return 0;
  ...
  string current = "";
  current.push_back(msg[n]);
  for (int i = n; i > n - 3; i--) {
    if (msg[i] == '0') break;
    // cout << current << endl;
    if (i == n) {
      ans += words.count(current);
    } else {
      current.insert(0, 1, msg[i]);
    }

  }

  return dp[n] = ans + countWays(n - 3, msg, dp, words);
}
void solve(int tt){
  int n;
  while (cin >> n && n) {
    ...
    cout << countWays(code.size() - 1, code, dp, words) << endl;
  }
}
