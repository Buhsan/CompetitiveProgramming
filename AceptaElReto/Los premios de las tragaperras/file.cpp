...
void solve(int tt){
  ...
  while (cin >> n && n) {
    int minCurrentSum = 0;
    int maxCurrentSum = 0;
    int bestMaxSum = INT_MIN;
    int bestMinSum = INT_MAX;
    int totalSum = 0;
    ...
    for (int i = 0; i < n; i++) {
      cin >> x;
      totalSum += x;
      maxCurrentSum = max(x, maxCurrentSum + x);
      minCurrentSum = min(x, minCurrentSum + x);

      bestMaxSum = max(bestMaxSum, maxCurrentSum);
      bestMinSum = min(bestMinSum, minCurrentSum);
    }

    cout << max(bestMaxSum, totalSum - bestMinSum) << endl;
  }
}
