...
set<pii> dirs = {{-1, 0}, {1, 0}, {0, 1}, {0, -1}};
void solve(int tt){
  int ancho, alto;
  while(cin >> ancho && ancho && cin >> alto) {
    int n;
    cin >> n;
    alto += 2;
    ...
    while (!q.empty()) {
      ...
      for (pii dir : dirs) {
        pii nuevaPos = {actual.first + dir.first, actual.second + dir.second};

        if (nuevaPos.first >= 0 && nuevaPos.first < alto && nuevaPos.second >= 0 && nuevaPos.second < ancho) {
          if (!visitados.count(nuevaPos)) {
            q.push(nuevaPos);
