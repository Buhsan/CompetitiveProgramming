...
#define PLIST(list)                                                            \
  ...
  cout << endl;
// #define PLIST(a) cout << #a << ": "; for (int i = 0; i < a.size(); ++i) {
// cout << a[i] << " "; } cout<<endl; #define FILL(list) for (auto elem : list)
// cin>>elem #define FILL(list) for (int i = 0; i < list.size(); ++i) cin >>
// list[i]
#define FILL(list)                                                             \
  for (auto &elem : list)                                                      \
  cin >> elem
...
struct MinOp {
  ...
  T id() const { return LLONG_MAX; }
};

void make_set(vector<int> &nodoPadre, int v) { nodoPadre[v] = v; }
...
int repString(string k, string search) {
  int cnt = 0;
  REP(i, 0, k.size()) {
    if (k.substr(i, search.size()) == search)
      cnt++;
  }
  return cnt;
}
...
int prefix_sum(vint a, int l, int r) {
  ...
  pref_sum[0] = a[0];
  REP(i, 1, a.size()) { pref_sum[i] = pref_sum[i - 1] + a[i]; }
...
void solve(int tt) {
  ...
  while (cin >> n >> m) {
    ...
    priority_queue<State, vector<State>, function<bool(State, State)>> pq(
        [](const State &a, const State &b) { return a.coste > b.coste; });
    pq.push({0, 0});
    minDist[0] = 0;
    ...
    while (!pq.empty()) {
      ...
      if (current.pos == n - 1)
        break;
      // if (current.pos == n - 1) {
      //   if (current.coste > minDist[current.pos]) break;
      // }

      for (auto [vecino, coste] : adj[current.pos]) {
        int newCoste = coste + current.coste;
        if (newCoste < minDist[vecino]) {
          minDist[vecino] = newCoste;
          pq.push({vecino, newCoste});
        }
      }
    }
  }
