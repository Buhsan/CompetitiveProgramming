...
int minTiros(int score, vector<int> &dianas, vector<int> &dp, vector<int> &prev) {
  if (dp[score] != -1) return dp[score];
  
  dp[score] = INT_MAX;

  for (int i = 0; i < dianas.size(); i++) {
    if (score - dianas[i] >= 0 && minTiros(score - dianas[i], dianas, dp, prev) != INT_MAX) {
      if (minTiros(score - dianas[i], dianas, dp, prev) + 1 < dp[score]) {
        dp[score] = minTiros(score - dianas[i], dianas, dp, prev) + 1;
        prev[score] = dianas[i];
      }
    }
  }
...
void solve(int tt){
  ...
  while (cin >> score >> num) {
    vector<int> a(num);
    vector<int> prev(score + 1, -1);
    REP (i, 0, num) cin >> a[i];
    SORTDECR(a);
    vector<int> dp(score + 1, INT_MAX);
    dp[0] = 0;
    REP(i,1,score+1) {
      REPLIST(d, a) {
    ...
    int answer = dp[score];
    if (answer == INT_MAX) cout << "Imposible" << endl;
    else {
      cout << answer << ": ";
