...
void solve(int tt) {
  ...
  while (cin >> n && n) {
    ...
    REP (i, 0, n) {
      ...
      cin >> fechaInicio >> fechaFin;

      int mesInicio = stoi(fechaInicio.substr(0, 2));
      int diaInicio = stoi(fechaInicio.substr(3, 2));
      
      int mesFinal = stoi(fechaFin.substr(0, 2));
      int diaFinal = stoi(fechaFin.substr(3, 2));

      dias.push_back({mesInicio, diaInicio, mesFinal, diaFinal});

    }

    sort(dias.begin(), dias.end(), [](const Curso &x, const Curso &y) {
      if (x.mesInicio != y.mesInicio) return x.mesInicio < y.mesInicio;
      if (x.diaInicio != y.diaInicio) return x.diaInicio < y.diaInicio;
      if (x.mesFinal != y.mesFinal) return x.mesFinal < y.mesFinal;
      return x.diaFinal < y.diaFinal;
    });

    int cursos = 1;

    int mesActual = dias[0].mesInicio;
    int diaActual = dias[0].diaInicio;
    vector<Curso> cursosActuales;
    for (int i = 1; i < dias.size(); i++) {

      for (auto it = cursosActuales.begin(); it != cursosActuales.end(); ) {
        if (mesActual > cursosActuales[j].mesFinal
        || (mesActual == cursosActuales[j].mesFinal && diaActual > cursosActuales[j].diaFinal)) {
          cursosActuales.erase(it);
          it--;
        } 
      }

      if (cursosActuales.size() >= 4) continue;
      else {
        mesActual = dias[i].mesInicio;
        inicioActual = dias[i].diaInicio;
      }
