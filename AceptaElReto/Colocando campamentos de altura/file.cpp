...
int minGasto(int i, vint &altura, vint &costes, int k, vint &dp) {
  if (dp[i] != INT_MAX) return dp[i];

  dp[i] = minGasto(i - 1, altura, costes, k, dp) + costes[i]; // si construimos campamento

  for (int j = i - 1; j >= 0; j--) {
    if (altura[i] - altura[j] >= k) break;
    dp[i] = min(dp[i], minGasto(j, altura, costes, k, dp) + costes[i]);
  }
...
void solve(int tt){
  ...
  while (cin >> n) {
    ...
    int k;
    cin >> k;
    vint dp(n + 1, INT_MAX); //dp[i] = min gasto si terminas construyendo el campamento i

    int ans = INT_MAX;
    dp[0] = 0;

    // for (int i = 1; i <= n; i++) {
    //   dp[i] = dp[i - 1] + costes[i];
    ...
    //   // if (altura[n] - altura[i] < k) ans = min(ans, dp[i]), cout << ans << " en campamento " << i << endl;
    // }

    cout << minGasto(n, altura, costes, k, dp) << endl;
